...
st.set_page_config(page_title="Laporan Manajemen", layout="centered")
...
try:
    ...
    st.markdown(f"""
        <div style='display: flex; justify-content: center; align-items: center; margin-bottom: 15px; width: 100%; padding-top: 10px;'>
            <img src='data:image/png;base64,{img_b64}' style='max-width: 160px; height: auto; filter: drop-shadow(0 0 10px rgba(255,255,255,0.1));'>
        </div>
    """, unsafe_allow_html=True)
except FileNotFoundError:
    pass

render_neo_tokyo_header("EXPORT DATA OPERASIONAL", "Sistem Laporan Feeder PDP", "var(--nt-text-muted)", align="center")
st.divider()
...
# ==========================================
# Menghapus emoji dari tab untuk kesan enterprise
tab1, tab2, tab3 = st.tabs(["HARIAN", "RENTANG KUSTOM", "BULANAN"])
    ...
    with st.container(border=True):
        ...
        if st.session_state.active_trigger == "harian":
            if st.session_state.report_data:
                ...
                st.warning("NIHIL: TIDAK ADA DATA PADA TANGGAL INI.")
        ...
        if isinstance(rentang_tanggal, tuple) and len(rentang_tanggal) == 2:
            ...
            if st.session_state.active_trigger == "rentang":
                if st.session_state.report_data:
                    ...
                    st.warning("NIHIL: TIDAK ADA DATA PADA RENTANG TANGGAL TERSEBUT.")
        else:
    ...
    with st.container(border=True):
        ...
        if st.session_state.active_trigger == "bulanan":
            if st.session_state.report_data:
                ...
                st.warning(f"NIHIL: TIDAK DITEMUKAN RECORDS PADA PERIODE {bln_pilih.upper()} {thn_pilih}.")
